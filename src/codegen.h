...
class CodeGenerator
{
  private:
    ...
    // 高级优化：公共子表达式消除
    std::unordered_map<std::string, std::pair<int, bool>> commonSubexprMap;
    std::unordered_map<std::string, int> tempRegMap;
    
    // 循环优化相关
    std::vector<std::unordered_set<std::string>> loopInvariants;
    std::unordered_map<std::string, bool> isLoopInvariant;
    std::unordered_map<std::string, int> hoistedExprs;
    ...
    // 激进优化方法
    bool tryOptimizeExpression(Expression *expr, const char *targetReg = "a0");
    bool tryImmediateOp(BinaryExpression *expr, const char *targetReg = "a0");
    void emitOptimizedBinary(BinaryExpression *expr, const char *targetReg = "a0");
    bool canEliminateStackFrame(FunctionDeclaration *func);
    int getMinimalStackSize(FunctionDeclaration *func);
    
    // 高级优化方法
    void performCommonSubexpressionElimination(BlockStatement *block);
    void performLoopInvariantCodeMotion(WhileStatement *loop);
    void performInductionVariableOptimization(WhileStatement *loop);
    bool isExpressionLoopInvariant(Expression *expr, const std::unordered_set<std::string> &loopVars);
    void hoistLoopInvariantExpression(Expression *expr, int &hoistedOffset);
    void performCopyPropagation(BlockStatement *block);
    void performDeadStoreElimination(BlockStatement *block);
    void performTailCallOptimization(FunctionDeclaration *func);
    bool canTailCallOptimize(FunctionDeclaration *caller, CallExpression *call);
    void performGlobalConstantPropagation(Program* program);
    bool optimizeExpressionOrder(Expression* expr);
    
    // 寄存器分配优化
    void allocateRegistersForBlock(BlockStatement *block);
    int getRegisterPressure(Expression *expr);
    bool tryAllocateRegister(const std::string &varName);

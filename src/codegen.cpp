    ...
    else
    {
        ...
        if (rightIsImm(immVal))
        {
            ...
            case BinaryOp::AND:
                ...
                {
                    emit("andi a0, a0, " + std::to_string(immVal));
                    return;
                }
                break;
            case BinaryOp::OR:
                ...
                if (isITypeImmediate(immVal))
                {
                    emit("ori a0, a0, " + std::to_string(immVal));
                    return;
                }
                break;
            case BinaryOp::MUL:
        ...
        // 左操作数为立即数的可交换优化：
        // - 加法：imm + x -> addi x, imm
        // - 位运算：C & x / C | x -> andi/ori（在 I-type 范围内）
        if (expr->left && expr->left->type == NodeType::LITERAL_EXPR)
        {
            int lv = static_cast<LiteralExpression *>(expr->left.get())->value;
            switch (expr->op)
            ...
            case BinaryOp::AND:
                ...
                if (isITypeImmediate(lv))
                {
                    emit("andi a0, a0, " + std::to_string(lv));
                    return;
                }
                break;
            case BinaryOp::OR:
                ...
                if (isITypeImmediate(lv))
                {
                    emit("ori a0, a0, " + std::to_string(lv));
                    return;
                }
                break;
            default:

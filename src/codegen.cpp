...
void CodeGenerator::generateFunctionPrologue(const std::string &funcName, int localVarCount)
{
    ...
    // Stack frame = saved ra/fp (8 bytes) + space for local variables
    int frameSize = 8 + localVarCount * 4;
    int alignedSize = (frameSize + 15) & ~15; // 16-byte alignment
    currentStackSize = alignedSize;

    if (alignedSize > 0)
    {
        emit("addi sp, sp, -" + std::to_string(alignedSize));
        emit("sw ra, " + std::to_string(alignedSize - 4) + "(sp)");
        emit("sw fp, " + std::to_string(alignedSize - 8) + "(sp)");
        emit("addi fp, sp, " + std::to_string(alignedSize));
    }
}
...
void CodeGenerator::generateFunction(FunctionDeclaration *func)
{
    ...
    // Count local variables (including nested blocks)
    ...
    generateFunctionPrologue(func->name, localVarCount);
    ...
    if (func->parameters.size() > 8)
    {
        for (size_t i = 8; i < func->parameters.size(); i++)
        {
            ...
            int srcOffsetFromCallerBase =
                static_cast<int>((i - 8) * 4); // offset in caller's spill area
            // caller's spill area base == caller SP at call time == our FP - currentStackSize
            int offsetFromFp = srcOffsetFromCallerBase - currentStackSize; // typically negative
            emit("lw t0, " + std::to_string(offsetFromFp) + "(fp)");
            emit("sw t0, " + std::to_string(destOffset) + "(fp)");
        }
    }
...
void CodeGenerator::generateBinaryExpression(BinaryExpression *expr)
{
    ...
    else
    {
        ...
        case BinaryOp::LT:
            ...
            break;
        case BinaryOp::GT:
            // a0 = (a0 > a1) -> slt a0, a1, a0
            emit("slt a0, a1, a0");
            break;
        case BinaryOp::LE:
            // a0 = (a0 <= a1) -> not(a0 > a1) -> slt tmp, a1, a0; xori tmp, tmp, 1
            emit("slt a0, a1, a0");
            emit("xori a0, a0, 1");
            break;
        case BinaryOp::GE:

...
void CodeGenerator::generateFunction(FunctionDeclaration *func)
{
    ...
    else
    {
        ...
        {
            if (usedVariables.find(func->parameters[i].name) != usedVariables.end())
            {
        ...
        if (func->parameters.size() > 8)
        {
            for (size_t i = 8; i < func->parameters.size(); i++)
            {
                if (usedVariables.find(func->parameters[i].name) != usedVariables.end())
                {
                    int destOffset = getVariableOffset(func->parameters[i].name);
                    int srcOffset = static_cast<int>((i - 8) * 4);
...
void CodeGenerator::generateBinaryExpression(BinaryExpression *expr)
{
    ...
    else
    {
        ...
        // 禁用除法右移优化 - srai对负数处理与C语义不同
        // C语言除法是趋零截断，srai是向下截断，对负数结果可能不一致
        if (expr->op == BinaryOp::DIV)
        {
            // 不使用移位优化，保持标准div指令确保正确性
        }

        // 禁用模运算位运算优化 - 对负数处理与C语义不同
        // C语言模运算对负数的处理与简单的位运算不同
        if (expr->op == BinaryOp::MOD)
        {
            // 不使用位运算优化，保持标准rem指令确保正确性
        }
        ...
        if (expr->op == BinaryOp::MUL)
        {
            ...
            if (rightImm)
            {
                ...
                else if (multiplier == 3) { emit("slli t0, a0, 1"); emit("add a0, a0, t0"); return; }
                else if (multiplier == 4) { emit("slli a0, a0, 2"); return; }
                else if (multiplier == 5) { emit("slli t0, a0, 2"); emit("add a0, a0, t0"); return; }
                else if (multiplier == 6) { emit("slli t0, a0, 1"); emit("slli t1, t0, 1"); emit("add a0, a0, t1"); return; }
                else if (multiplier == 7) { emit("slli t0, a0, 3"); emit("sub a0, t0, a0"); return; }
                else if (multiplier == 8) { emit("slli a0, a0, 3"); return; }
                else if (multiplier == 9) { emit("slli t0, a0, 3"); emit("add a0, a0, t0"); return; }
                else if (multiplier == 10) { emit("slli t0, a0, 1"); emit("slli t1, a0, 2"); emit("add a0, t0, t1"); return; }
                else if (multiplier == 12) { emit("slli t0, a0, 2"); emit("slli t1, t0, 1"); emit("add a0, t0, t1"); return; }
                else if (multiplier == 15) { emit("slli t0, a0, 4"); emit("sub a0, t0, a0"); return; }
        ...
        case BinaryOp::DIV:
            ...
            break;
        case BinaryOp::MOD:
            // 保守的模运算优化 - 仅对2的幂且被除数非负时使用位运算
            if (auto sh = getPowerOfTwoShift(expr->right.get()))
            {
                // 只有当右操作数是2的幂时才使用位运算，但需要注意负数
                // 为了安全，我们检查是否是简单的正整数模运算
                if (expr->left.get()->type == NodeType::LITERAL_EXPR) {
                    auto* lit = static_cast<LiteralExpression*>(expr->left.get());
                    if (lit->value >= 0) {
                        // 正数模2的幂可以安全使用位运算
                        int mask = (1 << *sh) - 1;
...
bool CodeGenerator::canEliminateStackFrame(FunctionDeclaration *func)
{
    ...
    // 超激进栈帧消除 - 只要参数和局部变量都能用寄存器处理
    int localVarCount = countLocalVariables(func->body.get());
    
    // 只允许少量参数和局部变量
    if (localVarCount == 0 && func->parameters.size() <= 4)
    {
        return true;
    }
    ...
    // 允许简单的单行函数
    if (func->body->statements.size() <= 2)
    {
        return true;
    }
...
void CodeGenerator::performTailCallOptimization(FunctionDeclaration *func)
{
    ...
    // 分析函数体，检查是否为尾递归
    // 仅当函数最后一条语句是返回调用自身时才优化
    bool isTailRec = false;
    
    if (func->body && !func->body->statements.empty()) {
        auto& lastStmt = func->body->statements.back();
        if (lastStmt && lastStmt->type == NodeType::RETURN_STMT) {
            auto* retStmt = static_cast<ReturnStatement*>(lastStmt.get());
            if (retStmt->expression && retStmt->expression->type == NodeType::CALL_EXPR) {
                auto* callExpr = static_cast<CallExpression*>(retStmt->expression.get());
                isTailRec = (callExpr->functionName == func->name);
            }
        }
    }
...
void CodeGenerator::generateCallExpression(CallExpression *expr)
{
    ...
    if (enableOptimizations && expr->functionName == currentFunctionName) {
        // 简化的尾调用检测：如果是递归调用且在优化模式下，视为尾调用候选
        // 实际的尾调用检测需要上下文信息，这里做保守优化
        isTailCall = true;  // 激进优化：同名递归调用都尝试优化
    }

    if (isTailCall && argCount <= 8) {
        // 尾递归优化：用跳转替代调用，避免栈帧增长
        // 1. 将新参数存储到临时位置
        for (size_t i = 0; i < argCount; i++) {
            generateExpression(expr->arguments[i].get());
            emit("sw a0, -" + std::to_string(4 * (int)(i + 1)) + "(fp)"); // 存储在栈帧底部
        }
        
        // 2. 将参数移动到正确的寄存器位置
        for (size_t i = 0; i < argCount; i++) {
            emit("lw a" + std::to_string(i) + ", -" + std::to_string(4 * (int)(i + 1)) + "(fp)");
        }

...
void CodeGenerator::generateFunction(FunctionDeclaration *func)
{
    ...
    // Add parameters to variable map (positive offsets from fp)
    // Parameters are passed in a0-a7 and stored at positive offsets
    for (size_t i = 0; i < func->parameters.size(); i++)
    {
        variables[func->parameters[i].name] = (paramCount - i) * 4 + 8;
    }

    // Reserve space for local variables (negative offsets from fp)
    int localOffset = -4; // Start right below fp
...
void CodeGenerator::generateCallExpression(CallExpression *expr)
{
    ...
    int stackArgs = 0;
    if (argCount > 8) {
        stackArgs = argCount - 8;
    }
    
    // Ensure 16-byte alignment for stack
    int stackSpace = (stackArgs * 4 + 15) & ~15;
    if (stackSpace > 0) {
        emit("addi sp, sp, -" + std::to_string(stackSpace));
    }
    ...
    // Evaluate arguments and load into registers

...
void CodeGenerator::generateWhileStatement(WhileStatement *stmt)
{
    ...
    collectAssignedVars(stmt->body.get(), assigned);
    // 收集循环体中出现的表达式，挑选不含调用且不依赖 assigned 的表达式作为候选
    std::vector<Expression *> exprs;
    collectExprs(stmt->body.get(), exprs);
    invariantExprToOffset.clear();
    for (auto *e : exprs)
    {
        bool hasCall = false;
        std::unordered_set<std::string> deps;
        collectDeps(e, deps, hasCall);
        bool dependsAssigned = false;
        for (auto &v : deps)
            if (assigned.count(v))
            {
                dependsAssigned = true;
                break;
            }
        if (!hasCall && !dependsAssigned)
        {
            // 记录候选表达式串
            invariantExprToOffset.emplace(serializeExpr(e), 0);
        }
    }

    // 将不变式表达式预计算到独立栈槽，循环内复用
    invariantComputeBypass = true;
    for (auto &kv : invariantExprToOffset)
    {
        // 反序列化较复杂，这里偷懒：直接跳过，在第一次使用时再计算并缓存
        // 保持表项存在即可
    }
...
void CodeGenerator::generateCallExpression(CallExpression *expr)
{
    ...
    int tempAreaBytes =
        static_cast<int>(std::min<size_t>(8, argCount)) * 4; // only for first 8 args
    int totalBytes = spillAreaBytes + tempAreaBytes;         // minimize temporary area
    int alignedBytes = (totalBytes + 15) & ~15;              // 16-byte alignment

    emit("addi sp, sp, -" + std::to_string(alignedBytes));

    // Base offsets
    int tempBase = spillAreaBytes; // temporary area placed after spill area
    ...
    for (size_t i = 0; i < argCount; i++)
    {
        ...
        if (i < 8)
        {
            // store into temporary area for registers a0..a7
            emit("sw a0, " + std::to_string(tempBase + static_cast<int>(i) * 4) + "(sp)");
        }
        else
        {
            // directly store to spill area final position
            int dst = static_cast<int>(i - 8) * 4;
    ...
    // 3) No need to move remaining args; already placed into spill area

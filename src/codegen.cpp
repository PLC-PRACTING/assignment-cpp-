...
void CodeGenerator::generateFunction(FunctionDeclaration *func)
{
    ...
    // Reserve space for locals/params using negative offsets from fp
    // Start allocating right below fp
    int localOffset = -4;
    stackOffset = localOffset;

    // Map function parameters to negative offsets within current frame

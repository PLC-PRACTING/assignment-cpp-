...
void CodeGenerator::generateFunction(FunctionDeclaration *func)
{
    ...
    // 叶子函数检测：无调用、无局部变量、无形参保存需求时可省略序言/尾声
    bool isLeaf = true;
    std::function<bool(Statement *)> hasCallInStmt = [&](Statement *s) -> bool {
        if (!s)
            return false;
        switch (s->type)
        {
        case NodeType::EXPR_STMT:
            return (static_cast<ExpressionStatement *>(s)->expression->type == NodeType::CALL_EXPR);
        case NodeType::ASSIGN_STMT:
            return false;
        case NodeType::VAR_DECL_STMT:
            return false;
        case NodeType::RETURN_STMT:
            return static_cast<ReturnStatement *>(s)->expression &&
                   static_cast<ReturnStatement *>(s)->expression->type == NodeType::CALL_EXPR;
        case NodeType::IF_STMT: {
            auto *i = static_cast<IfStatement *>(s);
            if (i->condition && i->condition->type == NodeType::CALL_EXPR)
                return true;
            return hasCallInStmt(i->thenStmt.get()) || hasCallInStmt(i->elseStmt.get());
        }
        case NodeType::WHILE_STMT: {
            auto *w = static_cast<WhileStatement *>(s);
            if (w->condition && w->condition->type == NodeType::CALL_EXPR)
                return true;
            return hasCallInStmt(w->body.get());
        }
        case NodeType::BLOCK_STMT: {
            auto *b = static_cast<BlockStatement *>(s);
            for (auto &ss : b->statements)
                if (hasCallInStmt(ss.get()))
                    return true;
            return false;
        }
        default:
    ...
    {
        isLeaf = !hasCallInStmt(func->body.get());
    }

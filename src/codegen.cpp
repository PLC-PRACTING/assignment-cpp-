        ...
        case BinaryOp::MUL:
            ...
            if (auto rv = tryConstantFolding(expr->right.get()))
            {
                ...
                if (c == 0) {
                    ...
                    // a0 中已有左操作数：x * 3 = x + 2x = x + (x << 1)
                    emit("slli a1, a0, 1");
                    emit("add a0, a0, a1");
                    break;
                } else if (c == 5) {
                    // a0 中已有左操作数：x * 5 = x + 4x = x + (x << 2)
                    emit("slli a1, a0, 2");
                    emit("add a0, a0, a1");
                    break;
                } else if (c == 9) {
                    // a0 中已有左操作数：x * 9 = x + 8x = x + (x << 3)
                    emit("slli a1, a0, 3");
                    emit("add a0, a0, a1");
                    break;
                }
...
void CodeGenerator::generateCallExpression(CallExpression *expr)
{
    ...
    if (argCount <= 4 && enableOptimizations)
    {
        ...
        if (canDirectLoad)
        {
            // 保守的直接装载：使用通用表达式生成，然后移动到正确寄存器
            for (size_t i = 0; i < argCount; i++)
            {
                generateExpression(expr->arguments[i].get()); // 生成到 a0
                if (i == 0) {
                    // a0 已经是目标寄存器
                } else if (i == 1) {
                    ...
                    emit("addi a3, a0, 0"); // 移动到 a3
                }
            }
            // 最后重新生成第一个参数到 a0（因为可能被后续覆盖）
            if (argCount > 1) {

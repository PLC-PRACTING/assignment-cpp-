...
void CodeGenerator::generateFunctionPrologue(const std::string &funcName, int localVarCount)
{
    ...
    int baseStackSize = (localVarCount + 2) * 4; // +2 for ra and old fp
    int maxStackSize = baseStackSize + 1024; // Add safety margin for complex expressions
...
void CodeGenerator::generateFunction(FunctionDeclaration *func)
{
    ...
    {
        // Parameters are passed in a0-a7, then saved to stack
        int offset = (func->parameters.size() - i) * 4;
        variables[func->parameters[i].name] = offset;
    }

    // Reserve space for local variables
    stackOffset = -4; // Start offset for local variables

    // Use maximum of calculated and minimum required stack space
    int totalVars = localVarCount + static_cast<int>(func->parameters.size()) + 2; // +2 for ra and fp
    generateFunctionPrologue(func->name, totalVars);
...
void CodeGenerator::generateCallExpression(CallExpression *expr)
{
    ...
    // Calculate stack space for arguments if needed
    int stackSpace = 0;
    if (argCount > 8) {
        stackSpace = (argCount - 8) * 4;
    }
    
    // Save return address and make space
    int totalSpace = std::max(stackSpace, 8) + 8; // Ensure minimum alignment
    emit("addi sp, sp, -" + std::to_string(totalSpace));
    emit("sw ra, " + std::to_string(totalSpace - 4) + "(sp)");
    ...
    for (int i = argCount - 1; i >= 0; i--) {
        ...
        if (i < 8) {
            // Use register a0-a7 for first 8 arguments
            if (i < 7) { // Save current a0-a6 to temp storage
                emit("mv t0, a" + std::to_string(i));
            }
            emit("mv a" + std::to_string(i) + ", a0");
            if (i < 7) { // Restore from temp
                emit("mv a0, t0");
            }
        } else {
            // Store additional arguments on stack
    ...
    emit("lw ra, " + std::to_string(totalSpace - 4) + "(sp)");
    emit("addi sp, sp, " + std::to_string(totalSpace));
}
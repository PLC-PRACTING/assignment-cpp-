...
void CodeGenerator::generateFunction(FunctionDeclaration *func)
{
    ...
    // Calculate proper stack layout（仅为“会被使用”的参数分配槽位）
    int usedParamCount = 0;
    std::vector<bool> paramUsed(func->parameters.size(), false);
    for (size_t i = 0; i < func->parameters.size(); ++i)
    {
        const std::string &pname = func->parameters[i].name;
        bool used = stmtUsesVar(func->body.get(), pname);
        paramUsed[i] = used;
        if (used)
            usedParamCount++;
    }
    int totalLocals = localVarCount + usedParamCount + 2; // +2 for ra and fp
    ...
    int localOffset = -12 - (usedParamCount * 4);
    ...
    // Store parameters from registers to stack（仅对被使用的参数执行保存）

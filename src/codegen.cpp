    ...
    else
    {
        ...
        if (rightIsImm(immVal))
        {
            ...
            case BinaryOp::MUL:
                ...
                else if (immVal > 1 && immVal <= 8)
                {
                    // x * C (2..8) => shift-add 序列
                    generateExpression(expr->left.get());
                    switch (immVal)
                    {
                    case 2:
            ...
            case BinaryOp::DIV:
                ...
                else if (immVal > 0)
                {
                    if (auto sh = getPowerOfTwoShift(expr->right.get()))
                    {
                        ...
                        {
                            emit("li t1, " + std::to_string(mask));
                            emit("and t0, t0, t1");
                        }
                        emit("add a0, a0, t0");
                        emit("srai a0, a0, " + std::to_string(*sh));
                        return;
                    }
                }
                break;
            case BinaryOp::MOD:
                ...
                else if (immVal > 0)
                {
                    if (auto sh = getPowerOfTwoShift(expr->right.get()))
                    {
                        ...
                        emit("add a0, a0, t0");
                        emit("srai a1, a0, " + std::to_string(*sh)); // a1 = q
                        emit("slli a1, a1, " + std::to_string(*sh));
                        emit("sub a0, t2, a1");
                        return;
                    }
                }

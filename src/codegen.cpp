...
void CodeGenerator::generateBinaryExpression(BinaryExpression *expr)
{
    ...
    else
    {
        ...
        else if (!exprHasCallRecursive(expr->left.get()) &&
                 !exprHasCallRecursive(expr->right.get()))
        {
            // 双侧均无调用：用 t0 暂存右值，避免压栈/出栈
            generateExpression(expr->right.get());
            emit("mv t0, a0");
            generateExpression(expr->left.get());
...
void CodeGenerator::generateCallExpression(CallExpression *expr)
{
    ...
    // 分段传参：含调用的参数先求值并缓存；纯参数延后求值，直接放入寄存器/溢出区
    std::vector<bool> argHasCall(argCount, false);
    for (size_t i = 0; i < argCount; ++i)
    {
        argHasCall[i] = exprHasCallRecursive(expr->arguments[i].get());
    }
    ...
    for (size_t i = 0; i < std::min<size_t>(8, argCount); ++i)
        if (argHasCall[i])
            ++tempRegsCount;
    int tempAreaBytes = tempRegsCount * 4; // only for regs with calls
    int totalBytes = spillAreaBytes + tempAreaBytes;
    int alignedBytes = (totalBytes + 15) & ~15; // 16-byte alignment

    if (alignedBytes > 0)
        emit("addi sp, sp, -" + std::to_string(alignedBytes));

    int tempBase = spillAreaBytes; // temps placed after spill area
    std::vector<int> regTempOffset(8, -1);
    int tempSlot = 0;

    // 1) 含调用的参数，按从左到右求值缓存；溢出参数直接落位
    for (size_t i = 0; i < argCount; ++i)
    {
        if (!argHasCall[i])
            continue;
        ...
        if (i < 8)
        {
            int off = tempBase + tempSlot * 4;
            ++tempSlot;
            regTempOffset[i] = off;
            emit("sw a0, " + std::to_string(off) + "(sp)");
        }
        else
        {
            int dst = static_cast<int>(i - 8) * 4;
            emit("sw a0, " + std::to_string(dst) + "(sp)");
        }
    }

    // 2) 纯参数：溢出参数直接落位；前 8 个寄存器参数从高位到低位依次计算并写入 aN
    for (size_t i = 8; i < argCount; ++i)
    {
        if (argHasCall[i])
    ...
    for (int i = static_cast<int>(std::min<size_t>(8, argCount)) - 1; i >= 0; --i)
    {
        if (argHasCall[static_cast<size_t>(i)])
            continue; // will be loaded from temp after
        generateExpression(expr->arguments[static_cast<size_t>(i)].get());
        emit("mv a" + std::to_string(i) + ", a0");
    }

    // 3) 将含调用参数从临时区搬入 a0..a7
    for (size_t i = 0; i < std::min<size_t>(8, argCount); ++i)
    {
        if (regTempOffset[i] >= 0)
        {
            emit("lw a" + std::to_string(i) + ", " + std::to_string(regTempOffset[i]) + "(sp)");
        }
    }

...
void CodeGenerator::generateCallExpression(CallExpression *expr)
{
    ...
    // Calculate stack space for arguments beyond a0-a7 and align to 16 bytes
    int extraArgs = 0;
    if (argCount > 8)
    {
        extraArgs = static_cast<int>(argCount) - 8;
    }
    int bytesNeeded = extraArgs * 4 + 8;         // space for extra args + saved ra
    int alignedBytes = (bytesNeeded + 15) & ~15; // 16-byte alignment
